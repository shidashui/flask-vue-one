export default {
  ...
  state:{
    ...
    // 用户未读消息计数
    new_messages_count: window.localStorage.getItem('codershui-token') ? JSON.parse(atob(window.localStorage.getItem('codershui-token').split('.')[1])).new_messages_count : 0
  },
