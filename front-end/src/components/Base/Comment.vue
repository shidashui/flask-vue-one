<template>
    <div class="media g-mb-20">
      <router-link v-bind:to="{path:`/user/${comment.author.id}`}" v-bind:title="comment.author.name||comment.author.username">
        <img class="d-flex g-width-50 g-height-50 rounded-circle g-mt-3 g-mr-20" v-bind:src="comment.author._links.avatar"
             v-bind:alt="comment.author.name||comment.author.username">
      </router-link>
      <div class="media-body g-brd-around g-brd-gray-light-v4 g-pa-20">
        <div class="d-sm-flex justify-content-sm-between align-items-sm-center g-mb-15 g-mb-10--sm">
          <h5 v-if="comment.author.id == comment.post.author.id" class="h4 g-font-weight-300 g-mr-10 g-mb-5 g-mb-0--sm g-color-pink">
            {{comment.anchor.name||comment.author.username}} <span class="h6">(作者)</span>
          </h5>
      ...
      v-highlight 是自定义指令，用 highlight.js 语法高亮 -->
        ...
        <div class="d-flex justify-content-start">
          <ul class="list-inline mb-0">
            ...
            <!-- 该评论发表在哪篇博客文章下面 -->
            <li class="list-inline-item g-mr-20">
              <router-link v-bind:to="{ name: 'PostDetail', params: { id: comment.post.id } }"
